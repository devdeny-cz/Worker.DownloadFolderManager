...
namespace DownloadFolderManager
{
    public class Worker : BackgroundService
    {
        ...
        public override Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            // run mapping device - window service is running as "superadmin" user and he dont can see mapped network device 
            RunMapNetworkDevice();
        ...
        private void RunMapNetworkDevice()
        {
            string mappingPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "mapping.bat");
            if (File.Exists(mappingPath))
            {
                _logger.LogInformation("Will be run mapping.bat");
                Process.Start(mappingPath);
            }
            else
            {
                _logger.LogInformation("Not exist mapping.bat");
            }
        }
    }
}
